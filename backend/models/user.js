...
function updateUser(params, callback) {
  var sql =
    "UPDATE users SET name=?, password=?, image=?, activity=? WHERE uid=?";
  db.execute(
    sql,
    [params.name, params.password, params.image, params.activity, params.uid],
    function (flag) {
      callback(flag);
    }
  );
...
function findFriend(params, callback) {
  var sql = "SELECT * FROM users WHERE uid !=  ? ";
  for (var i = 1; i < params.length; i++) sql += "AND uid != ? ";
...
function findFriendlistFriends(uid, callback) {
  var sql =
    "SELECT DISTINCT users.uid, users.name, users.image, users.activity FROM users, requests WHERE ((requests.sender = ? OR requests.receiver = ?) AND requests.status LIKE ? AND users.uid != ?) AND (users.uid = requests.sender OR users.uid = requests.receiver)";
...
function NoMessageSenderFriends(uid, callback) {
  var sql =
    "SELECT * FROM users INNER JOIN requests ON users.uid = requests.sender OR users.uid = requests.receiver WHERE users.uid != ? AND (requests.sender = ? OR requests.receiver = ?) AND requests.status LIKE ? AND users.uid NOT IN(SELECT * FROM(SELECT DISTINCT users.uid FROM users INNER JOIN(SELECT * FROM messages WHERE mid in( SELECT max(mid) as m FROM messages WHERE messages.receiver = ? or messages.sender = ? GROUP BY messages.sender,messages.receiver DESC))AS b ON users.uid = b.receiver or users.uid = b.sender WHERE users.uid != ? ORDER BY b.mid DESC)t GROUP BY uid) ORDER BY users.uid DESC";
  db.getResult(
    sql,
    [uid, uid, uid, "accepted", uid, uid, uid],
    function (result) {
      callback(result);
    }
  );
